...
def run_IGV_script(igv_script, igv_jar, memMB):
    ...
    igv_command = """nohup /gpfs1/RD_project/software/RD_software/xvfb/x86_64-conda_cos6-linux-gnu/sysroot/usr/bin/Xvfb :0 -nolisten tcp -fp /gpfs1/RD_project/software/RD_software/igv_snapshot/IGV-snapshot-automator/lib/X11/fonts/misc &\nDISPLAY=:0\njava -Xmx{}m -jar {} -b {} \n""".format(memMB,igv_jar, igv_script)
    igv_command += """ps -aux|grep Xvfb|grep zhaohon| grep -v grep | awk '{print $2}' |while read pid; do  echo "xvfb is running, to kill bootstrap pid=$pid"; kill -9 $pid; done"""
    print('\nIGV command is:\n{}\n'.format(igv_command))
...
def main(input_files, region_file = 'regions.bed', genome = 'hg19',
         ...
         igv_jar_bin = "/gpfs1/RD_project/software/RD_software/igv_snapshot/IGV-snapshot-automator/bin/IGV_2.3.81/igv.jar", igv_mem = "4000",
...
def run():
    ...
    parser.add_argument("-ht", default = '1500', type = str, dest = 'image_height', metavar = 'image height', help="Height for the IGV tracks")
